...
- [ ] **Step 7.2** Implement the full sequence (extracted into `executeFullApply`):
  ...
  7. **Stage record in memory** — do NOT write yet. Build `{ _id: name, version, appliedAt, appliedBy, manifest, resources, driftAccepted: [...existingDecisions] }`.
     ...
     **Critical: carry forward existing `driftAccepted`.** Phase 1's `applied-frames-store.upsert(record)` is `replaceOne` (see `applied-frames-store.ts`), which means a fresh upsert wipes the entire document — including the per-decision `driftAccepted` entries the dialog (Task 6) just wrote with `appendDriftDecision`. On the same-version drift path, those decisions ARE the durability promise the spec makes ("Decisions persist mid-session"). To preserve them: at the start of the same-version drift branch in step 3 (before running the dialog), read `existingDecisions = (existing.driftAccepted ?? [])`. The dialog appends new decisions; in step 7, copy `existingDecisions` plus any decisions the dialog appended (read the record again right before staging, OR have the dialog return the new decisions and concatenate them into `existingDecisions`) into the staged record's `driftAccepted` field. Net effect: re-applies preserve every decision ever made on this frame at this version. Fresh applies and version-bumps start with `driftAccepted: []`.
  8. **Post-apply hook** — identical invocation pattern to step 4 (path-containment check + binary-and-args `execFileSync` with `/bin/sh`). On non-zero exit: run reverse-best-effort over what was just written (call each `asset-writer.remove*` for the staged resources), collect errors. Throw `PartialApplyError` if reversal itself fails. **Both the `written` and `unreversed` arrays on `PartialApplyError` use `resourceKey()` strings as labels** — same canonical form as `forceWriteResources` and `DriftFinding.resource`, so operators see one consistent identifier across drift, dialog, audit output, and error messages. `unreversed` entries should append the underlying error message in parentheses (e.g., `"skills:memory-hygiene (ENOENT: bundle dir disappeared)"`).
